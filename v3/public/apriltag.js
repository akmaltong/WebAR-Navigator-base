...
class Apriltag {
    ...
    constructor(onDetectorReadyCallback) {
        ...
        // detector options
        this._opt = {
            // Decimate input image by this factor (1=no decimation, better quality)
            quad_decimate: 1.0,
            // Gaussian blur sigma
            quad_sigma: 0.0,
            // CPU threads (no effect in WASM)
            nthreads: 1,
            // Spend more time aligning edges
            refine_edges: 1,
            // Maximum detections (0=all)
            max_detections: 0,
            // Return pose: 0=disabled (no camera params needed)
            return_pose: 0,
            // Return pose solutions
            return_solutions: 0
        }
    ...
    detect(grayscaleImg, imgWidth, imgHeight) {
        ...
        console.log('AprilTag JSON:', detectionsJson);
